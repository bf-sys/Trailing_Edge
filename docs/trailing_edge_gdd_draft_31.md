...
- Abilities on number-key hotkeys. **Exception (2026-08-14 ability rework, implemented — §7):** `teleport`'s hotkey arms an aiming state rather than firing immediately; **left-click** confirms the destination and fires (originally the right mouse button, switched during playtesting — the browser's native context menu, which nothing in this codebase suppresses, made right-click unreliable to actually use). This is the only ability whose activation isn't a single keypress, since it's the only one needing a spatial target.
...
**Reframe (decided 2026-08-14, implemented and playtested — see
...
- **`teleport`** — reworked into the **Meso/Exploration** pillar's
  ...
  ring shows the max-distance limit, **left-click** confirms (originally
  right-click, switched after playtesting surfaced the browser's native
  context-menu conflict, §4) — if aimed beyond the ring, the destination clamps to the ring's edge and
...
**Ability-unlock info popup (decided 2026-08-14, implemented as
`AbilityUnlockScene`):** see §11.8 for the Scene-flow mechanics. Content
(what each popup actually says) lives in `abilityUnlockContent.ts`, not
authored as part of this GDD pass.
...
**Open-world hazards — one parameterized class, not five:** all four "zone" hazards from §9's table (Debris Field, Solar Flare, Ion Storm, Nebula Field) differ only in authored parameters, not in code. Recommend a single `HazardZoneElement` taking: `shape`, `movementPattern: 'static' | 'linear' | 'patrol'`, `speed`, `activation: 'continuous' | 'pulsed' | 'impact'` (`'impact'` added 2026-08-21 — a one-time hit on contact, gated by `hitCooldownSeconds`, rather than a per-second/per-pulse rate), `pulseIntervalSeconds`, `hitCooldownSeconds`, `resourceCost: { energy, structure }`, and (added 2026-08-07) `blocksMovement: boolean`. Meteoroid also fits this class (`movementPattern: 'linear'`, structure cost) rather than needing a separate moving-hazard-object type, since Arcade's overlap callbacks handle "zone overlap" and "moving hazard contact" identically at the interface level — a moving `HazardZoneElement` and a stationary one differ only in whether `speed` is nonzero. **This collapses what could have been five hazard classes into one class + five content configs — confirmed as the approach going into Phase 1, since it changes what counts as "core" vs. "content" in §12 below.** Adding `blocksMovement` doesn't reopen that decision — it's still one class, just one more parameter axis, not a sixth class. Reserved, not yet designed: an optional rate-modifier field so a zone can slow/speed energy regen instead of (or in addition to) draining it directly (§5).
- **Hard rule:** `onHazardContact()` calls `ShipSurvivalComponent.consumeEnergy/consumeStructure` — never modifies resource values itself; enforce via the same private-field discipline as 11.1. **`blocksMovement: true` no longer implies zero contact cost (coupling removed 2026-08-21):** a hazard can be a solid Arcade collider *and* still charge a cost on contact — Meteoroid does both as of its 2026-08-21 collision rework (this table's "Dynamic (moving hazard, collision)" entry for Meteoroid was actually anticipating this; the code hadn't caught up to it until now). Whether a given `blocksMovement` hazard also charges a cost is a per-hazard content decision in `hazardConfig.ts`, not a rule of the class — Debris Field stays zero-cost today, but that's tunable, not structural.
...
- **Functions:** `isUnlocked(type): boolean`, `tryActivate(type): boolean` — checks cooldown (if authored `cooldownSeconds` > 0), duration (if authored `durationSeconds` > 0, added 2026-08-14 for `scan`, implemented as `isActive(type, nowMs)`), and energy cost (if authored `energyCost` > 0) via `ShipSurvivalComponent.consumeEnergy`; any gate no-ops when its field is 0, per §7's dual/triple-gate model. Cost stays a single flat, authored number per ability type (§11.7) — deliberately not computed per-activation (e.g. not scaled to a clicked `teleport` distance), so this stays all-or-nothing with no partial-spend exception.
- Cooldowns via Phaser's `Time.addEvent` or a timestamp comparison against `scene.time.now` — don't build a second timer system.
- **`tractorBeam` exception (2026-08-14 ability rework, implemented):** `isUnlocked('tractorBeam')` returns `true` unconditionally rather than consulting `ProgressionManager` — it's deliberately excluded from the unlock sequence below (§7), not merely reordered within it. Any per-ability display logic (ability icons, §11.10; the unlock popup, §11.8) iterates `abilityUnlockOrder` (three entries) rather than every key of the ability-cost config, so `tractorBeam` never surfaces in player-facing UI.

### 11.4a `ExplorationController` — ability-driven movement effects (added 2026-08-14, implemented and playtested)
...
- **`rocketBoost`:** on activation, captures the ship's current heading from `ship.rotation` (already kept continuously in sync with velocity direction while moving, and simply retained while stopped — no new heading-tracking needed; the capture corrects for `shipConfig.spriteFacingOffsetRadians`, since `rotation` is the sprite's visual facing, not the raw travel angle) and drives velocity directly along it for `boostDurationSeconds`, overriding the normal click-to-move target-seeking calculation. Clicks/drags during the boost window keep updating the click-to-move target in the background exactly as they do outside a boost — nothing needs to suppress input — so movement resumes toward the current target the instant the boost ends, with no dead input period (confirmed in playtesting). **Implementation note, resolved:** the ship's Arcade body has a `setMaxVelocity()` cap set once at construction — `boostSpeed` above the ship's normal max speed is explicitly raised for the boost window via `setMaxVelocity(boostSpeed)` and restored to `shipConfig.maxSpeed` once the window ends. Boosting into a `blocksMovement: true` hazard is simply stopped by the hazard's existing solid collider, same as ordinary movement — energy already spent, no refund; this is deliberate (§7), confirmed in playtesting (a boost into the Debris Field stops dead at its edge), not a bug to fix later.
- **`teleport`:** on hotkey press, enters an aiming state that suppresses normal click-to-move input; a range ring (radius = the ability's fixed max distance, §7), implemented as `TeleportRangeRing`, renders centered on the ship, procedurally drawn the same way as `ShipStatusArcs`/the objective-marker texture (§11.10a, §11.10) rather than a new sprite asset. **Left-click confirms** (originally the right mouse button — switched during playtesting since the browser's native context menu, unsuppressed anywhere in this codebase, made right-click unreliable to actually use, §4): if the cursor is within the ring, the ship blinks directly there; if beyond it, the destination clamps to the ring's edge along that direction and the confirm reticle is drawn at the clamped point, not the raw cursor position, so the player always sees exactly where they'll land before committing. Passes through solid colliders via a plain position set (no tween/physics move) — the one ability that can cross a `blocksMovement: true` hazard (§7, §9's Debris Field entry); confirmed in playtesting by teleporting through a Debris Field into the exact center of a Nebula Field with no resistance.
...
- **`abilityUnlockOrder` is now three entries, not four** (2026-08-14 ability rework, implemented): `scan → teleport → rocketBoost`. `tractorBeam` is never granted through this sequence — see §11.4's exception above. Test Level (§9's vertical-slice test scene, kept out of `levelOrder`) force-grants all three on entry so its full hazard/ability surface is testable without first completing real levels.
...
- Per-ability `AbilityCostData { energyCost, cooldownSeconds }` — either field may be 0. **Extended 2026-08-14, implemented:** adds optional `durationSeconds` (scan), `maxRange` (teleport), and `boostSpeed`/`boostDurationSeconds` (rocketBoost) fields, siblings to the two above and following the same "may be 0/omitted where irrelevant" pattern. `teleport`'s fixed max range is a separate authored field on the same record, not derived from `energyCost` — the two are deliberately not linked by a cost-to-distance formula (§7).
...
Five Scenes as of 2026-08-14 (implemented), no level-select — progression is linear, the next level loads automatically on completion:
...
- **`AbilityUnlockScene`** (added 2026-08-14, implemented) — launched as a stacked overlay on the just-completed `GameScene`, same `scene.launch()` convention as `PauseScene` below rather than a new mechanism, showing what the just-granted ability does. Paused, no-time-pressure by design — dismissed only by an explicit close/exit button, never a timer or click-anywhere. Its close button performs the level transition `GameScene.handleLevelComplete()` deferred above via an `onClose` callback passed through as scene-launch data, the same pattern `GameScene` already uses for `levelId`, so the correct destination (next `levelId`, or `WinScene` if this was the last level) fires regardless of which one applies. Never launched for `tractorBeam` (§7, §11.4) — it isn't part of `abilityUnlockOrder`, so `grantNextAbility()` can never return it.
...
- Ability icons reflect `AbilityComponent.isUnlocked()`/cooldown state (11.4) — read-only from the HUD's side; it queries, it doesn't gate. **Source list changed 2026-08-14, implemented:** iterates `abilityUnlockOrder` (three entries, §11.5) rather than every key of the ability-cost config, so `tractorBeam` — dereferenced from player-facing ability UI (§7, §11.4) — never appears here.
...
- Off-screen objective marker: a single edge-pinned directional arrow, sourced from `LevelObjectiveTracker.getCurrentObjectiveTarget()` (§9's "Off-screen objective visibility," resolved 2026-07-31). **Amended 2026-08-14, implemented:** no longer unconditionally visible. Shown only while `scan`'s duration window is active (§7, `AbilityComponent.isActive('scan', ...)`), plus two energy-independent one-shot reveals via `flashObjectiveMarker()` — level start, and each `LevelObjectiveTracker` target change (probe found, beacon reached) — so a player is never left without a bearing at a moment the game just changed what it's asking of them, regardless of energy state. Rationale for why this doesn't reopen the disorientation problem the always-on version solved: objective locations are static within a phase, and the marker stays live for the full scan duration while the ship moves, so the player gets bearing-rate-of-change for free (motion parallax) — meaningfully more information than a single frozen reading — and can re-scan at will to refresh orientation after maneuvering. Also new alongside this: `HazardScanOverlay`, driven by the same `scan`-active window, drawing a structure/energy-colored outline plus a name label over nearby hazards (§9's Ion Storm/Nebula Field and structure-vs-energy mitigation notes).
...
- Structure renders as a horizontal bar below the ship; energy renders as a thinner horizontal bar directly beneath it (**switched from a curved arc 2026-08-14** — playtesting found the arc read as a shield, misrepresenting structure as absorbing damage rather than being consumed by it). Both track the ship's position every frame; neither rotates with the ship's heading, so the readout stays screen-upright while the hull turns.
...
- Procedurally drawn via `Phaser.GameObjects.Graphics` (two rects), not sprites — no art asset required, matching the precedent set by `HudOverlay`'s generated objective-marker texture.
...
**Ability rework amendment (decided 2026-08-14, implemented and
playtested the same day — full rationale in
`docs/ability-rework-brainstorm-2026-08-14.md`).** The
`AbilityComponent`/`ExplorationController`/`HudOverlay`/Scene-flow changes
in §7, §11.4, §11.4a, §11.5, §11.8, and §11.10 above are built: this
re-closes the slice of Phase 2a's core work the design pass had reopened
after it was already marked closed (2026-08-10/11). Scope was, as
originally stated, specifically the ability-mechanics surface (scan/
teleport/rocketBoost effects, `tractorBeam` dereferencing, the
ability-icon source list, the objective marker's visibility rule, and the
new `AbilityUnlockScene`) — it didn't touch the five `PuzzleElementBase`
subtypes, `SaveManager`, or `BootScene`/`TitleScene`/`PauseScene`/
`WinScene` beyond the one new Scene noted above. One implementation detail
surfaced only during playtesting, not anticipated in the design pass:
`teleport`'s confirm input moved from right-click to left-click, since the
browser's native context menu (nothing in this codebase calls
`preventDefault` on it) made right-click unreliable to actually use — see
§4 and §11.4a. Phase 2a is fully closed again as of this implementation.
