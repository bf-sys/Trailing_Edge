...
- Teleport — effect at origin and/or destination (mechanically implemented 2026-08-14 as `TeleportRangeRing`'s aim-time ring/reticle, but no blink-moment VFX) — **still open**
...
**Flag:** Teleport's blink-moment VFX is the one item of the original four still genuinely open — not named as an art requirement anywhere in §7 or §11, implied only by the ability existing at all. Scan and Rocket boost are now resolved (see above); Tractor/Repulsor stays de-scoped per the 2026-08-14 rework.
...
- Ability activation VFX — **partially resolved**: Scan (2026-08-26) and Rocket Boost (2026-08-24) now have dedicated activation VFX, see §1.5. Teleport's blink-moment VFX is the one item still open; Tractor/Repulsor stays de-scoped.
