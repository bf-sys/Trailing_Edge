...
- [ ] **Resupply waypoint during scan** — add a screen-edge marker, in a different color from the objective marker, pointing to the nearest AsteroidField resupply point while `scan` is active — a clear answer to "where do I go to heal." Note: `HudOverlay`'s existing off-screen marker only tracks the level's current *objective* (probe/beacon/exit), not resupply points, so this is a new marker type to design, not a tweak to the existing one.
